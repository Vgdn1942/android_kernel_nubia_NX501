...
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
{
	...
	if (chg_full && percent_soc <= 99 && is_charger_online() && !is_chg_batt_temp_abnormal()){
		chg_fsm = pm_chg_get_fsm_state(the_chip);
    ...
    /* As charging full and soc=100,after long time then soc=99,
           set the soc=100 when  batt_current<5mA and charger is still online   */
	if( chip->recent_reported_soc==100 && percent_soc==99 ){	
...
static void batt_monitor_worker(struct work_struct *work)
{
	...
	PMLOG_DEBUG("end=%d SOC=%d uV=%d fsm=%d I=%d T=%d\n",
			eoc_end,
...
void poweroff_charge_check(void)
{
	...
	batt_inst_irq = pm_chg_get_rt_status(the_chip, BATT_INSERTED_IRQ);
	batt_uv = get_prop_battery_uvolts(the_chip);
	if( batt_inst_irq || (batt_uv>BATT_MIN_UV && batt_uv<BATT_MAX_UV) )
		is_batt_on = 1;
...
#ifdef CONFIG_ZTEMT_ABNORMAL_BOOT
static int boot_status_reboot_reason = 0;

static ssize_t boot_status_show(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
	/*
	*1:normal_boot, longpress pwk bring up
	*0:abnormal boot
	*/
	return snprintf(buf, 32, "%u\n", (boot_status_reboot_reason == 0x00)? 0:1);
}
static DEVICE_ATTR(boot_status, 0444, boot_status_show, NULL);


int abnormal_boot_check(void)
{
	int ret = -1;
	static struct kobject *boot_status_kobject = NULL;

	boot_status_reboot_reason = __raw_readl(restart_reason);
	printk("%s reason:%x\n",__func__,boot_status_reboot_reason);

	boot_status_kobject = kobject_create_and_add("ztemt_boot_status", NULL);
	if(boot_status_kobject == NULL) {
		ret = -ENOMEM;
		goto out;
	}

	ret = sysfs_create_file(boot_status_kobject, &dev_attr_boot_status.attr);
	if(ret){
		goto sys_create_file_err;
	}
	return 0;

sys_create_file_err:
	kobject_del(boot_status_kobject);
out:
	return ret;
}
#endif
	...
	if (chip->update_time)
		schedule_delayed_work(&chip->update_heartbeat_work,
				      round_jiffies_relative(msecs_to_jiffies
							(chip->update_time)));
	#ifdef CONFIG_ZTEMT_ABNORMAL_BOOT
	//must be placed before poweroff_charge_check
	abnormal_boot_check();
	#endif
	#ifdef CONFIG_ZTEMT_POWEROFF_CHARGE
